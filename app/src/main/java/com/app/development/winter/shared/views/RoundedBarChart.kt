...
import com.app.development.winter.shared.extension.dpToPx
    ...
    private inner class RoundedBarChartRenderer internal constructor(
        ...
        override fun drawDataSet(c: Canvas, dataSet: IBarDataSet, index: Int) {
            ...
            if (mChart.isDrawBarShadowEnabled) {
                ...
                var x: Float
                var i = 0
                val count = Math.ceil((dataSet.entryCount.toFloat() * phaseX).toDouble()).toInt()
                ...
                while (i < count) {
                    ...
                    c.drawRoundRect(
                        mBarShadowRectBuffer, mRadius.toFloat(), mRadius.toFloat(), mShadowPaint
                    )
                    i++
                }
            }
            ...
            while (j < buffer.size()) {
                ...
                /* Stroke paint*//* mHighlightPaint.color = Color.parseColor("#1F1A1D")
                 c.drawRoundRect(
                     buffer.buffer[j],
                     15f.dpToPx(context),
                     buffer.buffer[j + 2],
                 ...
                 *//* Default paint*//*
                mHighlightPaint.color = Color.parseColor("#171A1F")
                c.drawRoundRect(
                    buffer.buffer[j].plus(0.5f.dpToPx(context)),
                    15.5f.dpToPx(context),
                    buffer.buffer[j + 2].minus(0.5f.dpToPx(context)),
                    buffer.buffer[j + 1].minus(0.5f.dpToPx(context)),
                    mRadius.toFloat(),
                    mRadius.toFloat(),
                    mHighlightPaint
                )
                ...
                c.drawRoundRect(
                    buffer.buffer[j],
                    buffer.buffer[j + 1].plus(6f.dpToPx(context)),
                    buffer.buffer[j + 2],
                    buffer.buffer[j + 3],
                    mRadius.toFloat(),
                    mRadius.toFloat(),
                ...
                mHighlightPaint.shader = LinearGradient(
                    buffer.buffer[j],
                    buffer.buffer[j + 3],
                    buffer.buffer[j],
                    buffer.buffer[j + 1],
                    context.getColor(R.color.colorGraphStrokeStart),
                    context.getColor(R.color.colorGraphStrokeEnd),
                    Shader.TileMode.MIRROR
                )
                c.drawRoundRect(
                    buffer.buffer[j],
                    buffer.buffer[j + 1],
                    buffer.buffer[j + 2].plus(1.25f.dpToPx(context)),
                    buffer.buffer[j + 3],
                    mRadius.toFloat() + 2,
                    mRadius.toFloat() + 2,
                    mHighlightPaint
                ...
                c.drawRoundRect(
                    buffer.buffer[j],
                    buffer.buffer[j + 1].minus(1.25f.dpToPx(context)),
                    buffer.buffer[j + 2],
                    buffer.buffer[j + 3].minus(1.25f.dpToPx(context)),
                    mRadius.toFloat(),
                    mRadius.toFloat(),
                    mRenderPaint
                )
